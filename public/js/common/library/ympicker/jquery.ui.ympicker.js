...
*/
        ...
        _generateHTML: function (inst) {
            ...
            today = this._daylightSavingAdjust(
                new Date(today.getFullYear(), today.getMonth(), 1)); // clear time
            var isRTL = this._get(inst, 'isRTL');
            ...
            for (var row = 0; row < numMonths[0]; row++) {
                ...
                for (var col = 0; col < numMonths[1]; col++) {
                    ...
                    for (var dRow = 0; dRow < 6; dRow++) { // create date picker rows
						if (dRow == 3)
						{
							calender += '<tr>';
                            calender += '<td colspan=4 align="center" class="ui-datepicker-year">' + printDate.getFullYear() + this._get(inst, 'yearSuffix') + '</td>';
...
//                                ((dow + firstDay + 6) % 7 >= 5 ? ' ui-datepicker-week-end' : '') + // highlight weekends
                            ...
                            tbody += (unselectable ? ' ' + this._unselectableClass + ' ui-state-disabled' : '');  // highlight unselectable days

                            tbody += (otherMonth && !showOtherMonths ? 
                                    '' : 
                            ...
                            tbody += ((!otherMonth || showOtherMonths) && daySettings[2] ? ' title="' + daySettings[2] + '"' : '')  // cell title

                            tbody += (unselectable ? 
                                    '' : 
                                    ' onclick="DP_jQuery_' + dpuuid + '.ympicker._selectDay(\'#' +
...
//                                            (otherMonth ? ' ui-priority-secondary' : '') + // distinguish dates from other months
    //							'" href="#">' + printDate.getDate() + '</a>')) + '</td>'; // display selectable date
                                    '" href="#">' + monthNamesShort[printDate.getMonth()] + '</a>'))
